...
def Demo():
    stairs = True
    x = 20
    PushF, LiftF, DownF, ForwardF, PushBW, LiftBW, DownBW, ForwardBW, PushLE, LiftLE, DownLE, ForwardLE, PushRI, LiftRI, DownRI, ForwardRI, LiftTactile = CalculationFastMotions()
    while stairs is True: 
        ...
        if  choice == "i":
            ...
            moveFast(PushF, LiftF,DownF,ForwardF)
            print("Move Forward")
        elif choice == "a":
            print("Move Left")
            moveFast(PushF, LiftLE, DownLE,ForwardLE)
        elif choice == "s": 
            print("Move backwards")
            moveFast(PushF, LiftBW, DownBW, ForwardBW)
        elif choice == "d":
            print("Move right")
            moveFast(PushF, LiftRI, DownRI, ForwardRI)
        elif choice == "t":
...
def CalculationFastMotions():
    ...
    return PushF, LiftF, DownF, ForwardF, PushBW, LiftBW, DownBW, ForwardBW, PushLE, LiftLE, DownLE, ForwardLE, PushRI, LiftRI, DownRI, ForwardRI, LiftTactile


def moveFast(PushBackwards, LiftUp, LiftDown, PutForward):
    ...
    pos1.extend(PutForward[30:36])
    positionN(pos1)
    check_position_error(20, 60, pos1)
    ...
    positionN(pos3)
    leg_case = [1,4,5]
    check_position_error_legs(40, 80, pos3, leg_case)

    pos4 = list()
    pos4.extend(PutForward[0:6])
    pos4.extend(PushBackwards[6:12])
    pos4.extend(PushBackwards[12:18])
    pos4.extend(PutForward[18:24])
    pos4.extend(PutForward[24:30])
    pos4.extend(PushBackwards[30:36])
    positionN(pos4)
    check_position_error(20, 60, pos4)

    pos5 = list()
    pos5.extend(LiftDown[0:6])
    pos5.extend(LiftDown[18:24])
    pos5.extend(LiftDown[24:30])
    positionN(pos5)
    leg_case = [1,4,5]
    check_position_error_legs(20, 60, pos5, leg_case)
