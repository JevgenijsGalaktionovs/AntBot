...
# OLD , NEEDS TO BE UPDATED with everything
def StandingInfiniteLoop(): #Infinite time
    # 1 servo ID 9
    INTERVAL = 0.1
    ...
    R_pos = [math.pi,3.66519165359,1.91986, #Leg 1 [pi, pi+40deg, pi-59.2deg]
	         math.pi,2.61799365359,4.36332, #Leg 2 [pi, pi-40deg, pi+59.2deg]
             math.pi,3.66519165359,1.91986, # list range from 0 to 17
             math.pi,2.61799365359,4.36332,
             math.pi,3.66519165359,1.91986,
             math.pi,2.61799365359,4.36332]
    ...
    while 1:
        ...
        for i in range(len(ID_list)):
            ...
            if servo_type == 1: # MX-28, coxa = i, femur= i+1, tibia=i+2
                ...
                E_pos[i+1] = R_pos[ID_list[i+1]-1] - all_pos_rad[ID_list[i+1]-1]
                E_pos[i+2] = R_pos[ID_list[i+2]-1] - all_pos_rad[ID_list[i+2]-1]
                E_vel[i]   = 0 - all_vel_radsec[ID_list[i]-1]
                E_vel[i+1] = 0 - all_vel_radsec[ID_list[i+1]-1]
                E_vel[i+2] = 0 - all_vel_radsec[ID_list[i+2]-1]
		...
		#print "MX-28 E_vel[i+1]: ", E_vel[i+1]
                ...
                T[i]  = M_11 * (E_pos[i]   * Kp_coxa  + 0 + 0) \
                      + M_12 * (E_pos[i+1] * Kp_femur + 0 + 0) \
                      + M_13 * (E_pos[i+2] * Kp_tibia + 0 + 0) \
                      + Friction + VG
                # if T[i] > 0 and all_vel_radsec[i] == 0:
                #     T[i] += coloumb_fr_mx28
                # elif T[i] < 0 and all_vel_radsec[i] == 0:
                #     T[i] -= coloumb_fr_mx28
                # print "M_11: ",M_11
                # print "M_12: ",M_12
                # print "M_13: ",M_13
                ...
                M_31 = CalcM31(all_pos_rad[i-1],all_pos_rad[i])
                ...
                T[i]  = M_31 * (E_pos[i-2] * Kp_coxa)  \
                      + M_32 * (E_pos[i-1] * Kp_femur) \
                      + M_33 * (E_pos[i]   * Kp_tibia) \
                      + VG + Friction
        ...
        # print "PWM:", servo_pwm_list
        error_in_degrees = [((x*180)/math.pi) for x in E_pos]
        # ae = [(x*4096.0)/2*math.pi for x in E_pos]
        error_rounded = [round(x, 3) for x in error_in_degrees]
        if error_rounded < 3:
            Kp_coxa =  1
            Kp_femur = 6
            Kp_tibia = 10
            ...
            Kp_femur = 3
            Kp_tibia = 5
        print "R_pos:", error_rounded[0]
        print sum(([abs(x) for x in error_rounded]))/len(error_rounded)
    ...
    print "Execution Time: " , end # IN DEVELOPMENT, WORKS ~ish fine only if R_a's will be 0.
def Standing(): # WORKS FOR ALL SERVOS but wrong numbers
    # 1 servo ID 9
    INTERVAL = 0.1
