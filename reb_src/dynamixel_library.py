...
ADDR_MX_prof_vel           = 112
ADDR_MX_prof_acc           = 108
...
def BulkWrite(ADDR_MX, DATA_LEN, list_18_values):
    ...
    for ID in range (1,len(list_18_values)+1):
...
def BulkWriteRippleGait(ADDR_MX, DATA_LEN, list_18_values, leg_case):
    value_byte_array = list()
    if   DATA_LEN == 1:
        for x in range(0,len(list_18_values)):
            value_byte_array.append([list_18_values[x]])
        ...
        for ID in range (1,len(list_18_values)+1):
            if 1<= ID <= 3 or 4 <= ID <= 6:
                ...
                if dxl_addparam_result != True:
                    ...
                    pass
    elif leg_case ==2:  # LEG 3,4
        for ID in range (1,len(list_18_values)+1):
            if  7<= ID <= 9 or 10 <= ID <= 12:
                dxl_addparam_result = gbw.addParam(ID, ADDR_MX, DATA_LEN, value_byte_array[ID-1])
                if dxl_addparam_result != True:
                    print("[ID:%03d] groupBulkRead addparam failed" % ID)
                    quit()
...
def BulkWriteTripodRotationy(ADDR_MX, DATA_LEN, list_18_values, leg_case):
    value_byte_array = list()
    if   DATA_LEN == 1:
        for x in range(0,len(list_18_values)):
            value_byte_array.append([list_18_values[x]])
    ...
    if leg_case == 1:  # LEG 1,3,5 (First Tripod Group)
        for ID in range (1,19):
            if 1<= ID <= 3 or 7 <= ID <= 9 or 13<= ID <= 15:
                dxl_addparam_result = gbw.addParam(ID, ADDR_MX, DATA_LEN, value_byte_array[ID-1])
                if dxl_addparam_result != True:
                    print("[ID:%03d] groupBulkRead addparam failed" % ID)
                    quit()
                    ...
                    pass
    else:  # LEG 2,4,6 (First Tripod Group)
        for ID in range (1,19):
            if 4 <= ID <= 6 or 10 <= ID <= 12 or 16 <= ID <= 18:
                dxl_addparam_result = gbw.addParam(ID, ADDR_MX, DATA_LEN, value_byte_array[ID-1])
                if dxl_addparam_result != True:
                    print("[ID:%03d] groupBulkRead addparam failed" % ID)
                    quit()
...
def BulkWriteParallel(ADDR_MX, DATA_LEN, list_18_values, leg_case):
    value_byte_array = list()
    if   DATA_LEN == 1:
        for x in range(0,len(list_18_values)):
            value_byte_array.append([list_18_values[x]])
...
def calc_Velocity(thetas):
    a = len(thetas)
    for i in range(0,a):
        velocity_list = [i*0.05 for i in thetas]
        velocity_int_list = [int(i) for i in velocity_list]
        return velocity_int_list
...
def WriteRippleGait(POS18VALUES_LIST,leg_case):
    BulkWriteRippleGait(ADDR_MX_GOAL_POSITION, LEN_4BYTES, POS18VALUES_LIST,leg_case)
def WriteProfAcc(ACC18VALUES_LIST):
    BulkWrite(ADDR_MX_prof_acc, LEN_4BYTES, ACC18VALUES_LIST)
def WriteProfVel(VEL18VALUES_LIST):
    BulkWrite(ADDR_MX_prof_vel, LEN_4BYTES, VEL18VALUES_LIST)
def WriteTripodGaitVel(VEL18VALUES_LIST,leg_case):
    BulkWriteTripodGait(ADDR_MX_prof_vel, LEN_4BYTES,VEL18VALUES_LIST,leg_case)
def WriteTripodGaitAcc(ACC18VALUES_LIST,leg_case):
    BulkWriteTripodGait(ADDR_MX_prof_acc, LEN_4BYTES,ACC18VALUES_LIST,leg_case)
def WriteParallel(POS18VALUES_LIST,leg_case):
    BulkWriteParallel(ADDR_MX_GOAL_POSITION, LEN_4BYTES, POS18VALUES_LIST,leg_case)
def WriteParallelVel(VEL18VALUES_LIST,leg_case):
    BulkWriteParallel(ADDR_MX_prof_vel, LEN_4BYTES,VEL18VALUES_LIST,leg_case)
def WriteParallelAcc(ACC18VALUES_LIST,leg_case):
    BulkWriteParallel(ADDR_MX_prof_acc, LEN_4BYTES,ACC18VALUES_LIST,leg_case)
def WriteRippleVel(VEL18VALUES_LIST,leg_case):
    BulkWriteRippleGait(ADDR_MX_prof_vel, LEN_4BYTES,VEL18VALUES_LIST,leg_case)
def WriteRippleAcc(ACC18VALUES_LIST,leg_case):
    BulkWriteRippleGait(ADDR_MX_prof_acc, LEN_4BYTES,ACC18VALUES_LIST,leg_case)
######################################
######################################
...
def writeProfVel(servo_id,value):
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, servo_id,ADDR_MX_prof_vel, value) # 16 - PWM Control Mode
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("%s" % packetHandler.getRxPacketError(dxl_error))

def writeProfAcc(servo_id,value):
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, servo_id,ADDR_MX_prof_acc,value) # 16 - PWM Control Mode
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("%s" % packetHandler.getRxPacketError(dxl_error))
